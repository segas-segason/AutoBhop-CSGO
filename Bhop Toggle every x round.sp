...
{
	abVelocity		=	CreateConVar("sm_ab_velocity",		"1", "Whethere to show velocity when bhop is enabled.");
...
public void OnBhop_RoundStart(Handle event, const char[] name, bool dontBroadcast)
{
	...
	//if(g_iround == 5 || g_iround == 10 || g_iround == 15 || g_iround == 20 || g_iround == 25 || g_iround == 30 || g_iround == 35 || g_iround == 40 || g_iround == 45 || g_iround == 50)
